  ...
  <div class="note">
    <v-card class="mb-4">
      ...
      <v-card-actions>
        <v-row>
          <v-col>
            <v-btn :to="`/editNote/${note.id}`" block variant="text" color="primary"> Düzenle</v-btn>
          </v-col>
          <v-col>
            <v-btn @click="dialog.delete = true" block variant="text" color="red"> Sil </v-btn>
          </v-col>
        </v-row>
      </v-card-actions>
    </v-card>
...
const charactersAmount = computed(() => {
  ...
  const text = length > 1 ? 'Karakter' : 'Karakter';
  return `${length} ${text}`;
});
